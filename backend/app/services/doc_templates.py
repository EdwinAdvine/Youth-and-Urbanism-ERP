...
class ERPTemplateEngine:
    ...
    async def generate_invoice_docx(self, invoice_id: uuid.UUID) -> tuple[bytes, str]:
        ...
        # Footer
        doc.add_paragraph("")
        footer = doc.add_paragraph("Generated by Urban ERP")
    ...
    async def generate_payslip_docx(
        ...
        doc.add_paragraph("")
        footer = doc.add_paragraph("Generated by Urban ERP — Confidential")
        footer.alignment = WD_ALIGN_PARAGRAPH.CENTER
        for run in footer.runs:
            run.font.color.rgb = RGBColor(0x99, 0x99, 0x99)
            run.font.size = Pt(8)
    ...
    async def generate_project_report_docx(self, project_id: uuid.UUID) -> tuple[bytes, str]:
        ...
        doc.add_paragraph("")
        footer = doc.add_paragraph(f"Generated by Urban ERP — {_fmt_date(date.today())}")
        footer.alignment = WD_ALIGN_PARAGRAPH.CENTER
        for run in footer.runs:
            run.font.color.rgb = RGBColor(0x99, 0x99, 0x99)
            run.font.size = Pt(8)
    ...
    async def generate_financial_report_xlsx(
        ...
        # Footer
        row += 1
        ws.cell(row=row, column=1, value=f"Generated by Urban ERP — {_fmt_date(date.today())}")
