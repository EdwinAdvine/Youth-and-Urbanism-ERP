...
async def list_invoices(
    ...
    invoice_data = [InvoiceOut.model_validate(i) for i in invoices]
    if fields:
        invoice_data = [apply_sparse_fields(inv, fields) for inv in invoice_data]
    return {
        "total": total,
...
async def create_invoice(
    ...
    await event_bus.publish_data_change("invoice", str(invoice.id), "created")
...
async def update_invoice(
    ...
    await db.commit()
    await db.refresh(invoice)
    await event_bus.publish_data_change("invoice", str(invoice.id), "updated")
    return InvoiceOut.model_validate(invoice).model_dump()
...
async def delete_invoice(
    ...
    await db.commit()
    await event_bus.publish_data_change("invoice", invoice_id_str, "deleted")
    return Response(status_code=status.HTTP_200_OK)
...
async def create_payment(
    ...
    await event_bus.publish_data_change("payment", str(payment.id), "created")
...
async def export_payments(
    ...
    columns = ["payment_number", "amount", "currency", "payment_method", "payment_date", "reference", "status", "created_at"]
...
async def generate_invoice_pdf(
    ...
    if invoice.items:
        for item in invoice.items:
            ...
            items_html += f"<tr><td style='padding:8px;border-bottom:1px solid #eee'>{desc}</td><td style='padding:8px;border-bottom:1px solid #eee;text-align:center'>{qty}</td><td style='padding:8px;border-bottom:1px solid #eee;text-align:right'>{price:,.2f}</td><td style='padding:8px;border-bottom:1px solid #eee;text-align:right'>{line_total:,.2f}</td></tr>"
...
.company {{ font-size: 24px; font-weight: bold; color: #51459d; }}
.invoice-info {{ text-align: right; }}
.invoice-number {{ font-size: 20px; font-weight: bold; color: #51459d; }}
table {{ width: 100%; border-collapse: collapse; margin-top: 20px; }}
th {{ background: #51459d; color: white; padding: 10px 8px; text-align: left; }}
th:nth-child(2), th:nth-child(3), th:nth-child(4) {{ text-align: right; }}
th:nth-child(2) {{ text-align: center; }}
.totals {{ margin-top: 20px; text-align: right; }}
.totals table {{ width: 300px; margin-left: auto; }}
.totals td {{ padding: 6px 8px; }}
.total-row {{ font-weight: bold; font-size: 16px; border-top: 2px solid #51459d; }}
...
<div class="header">
    <div>
        <div class="company">Urban Vibes Dynamics</div>
        <div style="color:#888;margin-top:4px">Invoice</div>
    </div>
...
<div class="totals">
    <table>
        <tr><td>Subtotal:</td><td style="text-align:right">{invoice.currency} {subtotal:,.2f}</td></tr>
        <tr><td>Tax:</td><td style="text-align:right">{invoice.currency} {tax:,.2f}</td></tr>
        <tr class="total-row"><td>Total:</td><td style="text-align:right">{invoice.currency} {total:,.2f}</td></tr>
    </table>
</div>

{f'<div style="margin-top:20px;padding:15px;background:#f8f8f8;border-radius:8px"><strong>Notes:</strong> {invoice.notes}</div>' if invoice.notes else ''}

<div class="footer">
    Generated by Urban Vibes Dynamics &bull; {datetime.now().strftime("%Y-%m-%d %H:%M")}
</div>
</body>
</html>"""

    # Return HTML as downloadable file (can be printed to PDF from browser)
    # For true PDF, we'd use weasyprint/wkhtmltopdf, but HTML is sufficient and zero-dependency
    content = io.BytesIO(html.encode("utf-8"))
