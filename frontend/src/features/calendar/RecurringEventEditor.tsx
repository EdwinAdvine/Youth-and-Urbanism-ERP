    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-gray-800 rounded-[10px] shadow-2xl w-full max-w-md">
        ...
        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Recurring Event</h3>
        ...
        <div className="p-5 space-y-5">
          {/* Frequency */}
          <div>
            <label className="text-xs font-medium text-gray-700 dark:text-gray-300 block mb-2">Repeat</label>
              ...
              {(['daily', 'weekly', 'monthly', 'yearly', 'custom'] as RecurrenceFrequency[]).map(
                (f) => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs rounded-[8px] border capitalize transition-colors ${ frequency === f ? 'bg-[#51459d] text-white border-[#51459d]' : 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800' }`}
                  >
                    {f}
                  </button>
                )
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-20 px-3 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-[8px] focus:outline-none focus:ring-1 focus:ring-[#51459d]/40"
              />
              ...
              <div className="flex gap-1">
                {WEEKDAY_LABELS.map((label, i) => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-full text-[11px] font-medium transition-colors ${ weekdays.includes(i) ? 'bg-[#51459d] text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200' }`}
                  >
                    {label.charAt(0)}
                  </button>
                ))}
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="text-[#51459d] focus:ring-[#51459d]/40"
                />
                <span className="text-xs text-gray-600 dark:text-gray-400">On date</span>
                {endType === 'on_date' && (
                  <input
                    type="date"
                    value={endDate}
                    onChange={(e) => setEndDate(e.target.value)}
                    className="ml-2 px-2 py-1 text-xs border border-gray-200 dark:border-gray-700 rounded-[6px] focus:outline-none focus:ring-1 focus:ring-[#51459d]/40"
                  />
                )}
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                {endType === 'after_count' && (
                  <>
                    <input
                      ...
                      className="w-16 px-2 py-1 text-xs border border-gray-200 dark:border-gray-700 rounded-[6px] focus:outline-none focus:ring-1 focus:ring-[#51459d]/40"
                    />
