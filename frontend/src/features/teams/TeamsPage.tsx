          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-2 text-xs rounded-[8px] border transition-colors ${startNow ? 'bg-[#51459d] text-white border-[#51459d]' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
              >
                Start now
              </button>
              <button
          ...
          {!startNow && (
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-[8px] focus:outline-none" />
              </div>
              <div>
  ...
  return (
    <div className="flex-1 flex flex-col h-full">
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 shrink-0">
        ...
        <button onClick={copyLink} className="flex items-center gap-1.5 text-xs border border-gray-200 text-gray-600 px-3 py-1.5 rounded-[8px] hover:bg-gray-50 transition-colors">
          {copied ? (
...
function MeetingCard({ meeting, onJoin, onViewSummary }: { meeting: Meeting; onJoin: (m: Meeting) => void; onViewSummary?: (id: string) => void }) {
  ...
  return (
    <div className={`bg-white border rounded-[10px] p-4 transition-all hover:shadow-md ${status === 'live' ? 'border-red-200 bg-red-50/30' : 'border-gray-100'}`}>
    ...
    <div className="h-full flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="flex border-b border-gray-100">
          {(['upcoming', 'past'] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`flex-1 py-2.5 text-xs font-medium capitalize transition-colors ${tab === t ? 'text-[#51459d] border-b-2 border-[#51459d]' : 'text-gray-500 hover:text-gray-700'}`}
            >
      ...
      <div className="flex-1 bg-gray-50 flex flex-col items-center justify-center p-4 sm:p-8">
        <div className="text-center max-w-md w-full">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">Urban Teams</h2>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button onClick={() => setShowNew(true)} className="flex flex-col items-center gap-2 p-4 bg-white border border-gray-200 rounded-[10px] hover:border-[#51459d]/40 hover:shadow-md transition-all min-h-[100px]">
              ...
              <div>
                <p className="text-xs font-semibold text-gray-800">New Meeting</p>
            ...
            <button
              ...
              className="flex flex-col items-center gap-2 p-4 bg-white border border-gray-200 rounded-[10px] hover:border-[#51459d]/40 hover:shadow-md transition-all"
            >
              ...
              <div>
                <p className="text-xs font-semibold text-gray-800">Join by Link</p>
                <p className="text-[10px] text-gray-400">Enter meeting ID</p>
              </div>
            </button>
          </div>

          <div className="bg-white border border-gray-100 rounded-[10px] p-4 text-left">
            <p className="text-xs font-semibold text-gray-700 mb-2">Powered by Jitsi Meet</p>
