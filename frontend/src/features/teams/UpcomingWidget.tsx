      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {meetings.map((meeting) => {
            ...
            return (
              <div key={meeting.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors">
                <div className={`w-1 h-10 rounded-full shrink-0 ${isNow ? 'bg-green-500' : 'bg-primary'}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-700 truncate">{meeting.title}</p>
