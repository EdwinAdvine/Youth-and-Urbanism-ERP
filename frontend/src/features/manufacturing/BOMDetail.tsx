    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{bom.bom_number}</h1>
      ...
      <Card className="mb-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
          <div>
            ...
            <p className="text-sm font-semibold text-gray-900 dark:text-gray-100 mt-1">{bom.name}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Finished Item</p>
            <p className="text-sm text-gray-900 dark:text-gray-100 mt-1">{bom.finished_item_name ?? bom.finished_item_id}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Qty Produced</p>
            <p className="text-sm text-gray-900 dark:text-gray-100 mt-1">{bom.quantity_produced}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Created</p>
            <p className="text-sm text-gray-900 dark:text-gray-100 mt-1">{formatDate(bom.created_at)}</p>
          </div>
        </div>
        {bom.notes && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Notes</p>
            <p className="text-sm text-gray-700 dark:text-gray-300">{bom.notes}</p>
          </div>
        )}
      </Card>
            ...
            {costLoading ? (
              ...
              <div className="flex items-center gap-6 mt-2">
                <div>
                  ...
                  <p className="text-lg font-bold text-gray-900 dark:text-gray-100">{formatCurrency(cost.unit_cost)}</p>
                </div>
                <div>
      ...
      <Card padding={false}>
        <div className="p-5 border-b border-gray-100 dark:border-gray-800">
          <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Materials ({bom.items.length} items)</h2>
        </div>
              ...
              {bom.items.length === 0 ? (
                ...
                bom.items
                  ...
                  .map((item: BOMItem, idx: number) => (
                    <tr key={item.id} className="border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                      <td className="py-3 px-4 text-gray-400">{idx + 1}</td>
                      <td className="py-3 px-4">
                        <span className="font-medium text-gray-900 dark:text-gray-100">{item.item_name ?? item.item_id}</span>
                      </td>
                      <td className="py-3 px-4 text-right font-medium">{item.quantity_required}</td>
                      <td className="py-3 px-4 text-gray-600 dark:text-gray-400">{item.unit_of_measure}</td>
